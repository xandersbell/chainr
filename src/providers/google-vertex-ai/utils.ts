...
/**
 * ADC (Application Default Credentials) credential file type definitions
 * authorized_user: User credentials generated by `gcloud auth application-default login`
 * service_account: Service account key downloaded from GCP console
 */
interface ADCAuthorizedUser {
  ...
  // quota_project_id is optional; some ADC files include it
...
/**
 * Get the ADC credential file path
 * Search order:
 * 1. GOOGLE_APPLICATION_CREDENTIALS environment variable
 * 2. Default path ~/.config/gcloud/application_default_credentials.json
 */
const getADCFilePath = (): string | null => {
  // Prefer the path specified by the environment variable
  const envPath = process.env.GOOGLE_APPLICATION_CREDENTIALS;
  if (envPath && fs.existsSync(envPath)) {
    return envPath;
  }

  // Fallback to the default gcloud path
...
/**
 * Read and parse the ADC credential file
 */
...
/**
 * Exchange refresh_token for access_token (authorized_user type)
...
/**
 * Get access_token via ADC
 * Automatically detect credential type and use the corresponding auth flow:
 * - authorized_user → refresh_token exchange
 * - service_account → JWT signing exchange (reuses getAccessToken)
 *
 * Returns { token, projectId } — projectId is only available for service_account type
 */
...
export const derefer = (
  ...
  if ('$ref' in node && typeof node.$ref === 'string') {
    ...
    if (defKey && target) {
      if (stack.has(defKey)) return { type: 'object' }; // Replace self-references with a generic object type
      stack.add(defKey);
...
// Use a whitelist to ensure unsupported fields are filtered out
...
// Recursively filter schema fields, keeping only Vertex AI supported fields
const filterSchemaFields = (obj: any, isSchemaObject: boolean): void => {
  if (typeof obj !== 'object' || obj === null || Array.isArray(obj)) return;

  // Only filter keys on schema objects (properties map is not filtered)
  if (isSchemaObject) {
    // Convert array type to anyOf format (Vertex AI does not support type arrays)
