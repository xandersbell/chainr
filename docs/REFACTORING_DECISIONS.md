...
**创建时间**: 2026-04-24
**最后更新**: 2026-04-24 13:26 EEST
**状态**: 🔄 实施中（Phase 1 已完成，Phase 2 进行中）
...
| Hono 依赖 | 6 个文件仍 import `Context from 'hono'`（bedrock、azure-openai、azure-ai-inference、sagemaker、google-vertex-ai） |
...
| `src/core/awsSigV4.ts` | 待删除，已被 `@smithy/signature-v4` 替代 |
...
| `src/providers/bedrock/utils.ts` | ⬜ 待修改 | 剥离 Hono Context（保留 @smithy） |
...
| `src/core/awsSigV4.ts` | ⬜ 待删除 | 被 @smithy 替代 |
