...
**创建时间**: 2026-04-24
**最后更新**: 2026-04-24 16:05 EEST
**状态**: 🔄 Phase 5 待实施 — TS 0 错误，384 测试通过，68 个 provider 已注册

---

## 1. 核心原则

**一句话总结**：完全采用 Portkey 的 Provider 架构（ProviderConfig 参数映射 + api.ts URL/headers + ResponseTransform），彻底替换 Chainr 早期的手写 switch-case 实现。
...
Portkey 的完整调用链（Chainr 需要对齐的目标架构）：
...
- `buildProviderRequest()` 已实现（providerRequest.ts），可构建完整请求（URL + headers + body）
- TS 错误：1279 → **0**
- 测试：384 个通过
...
| 缺失项 | 说明 |
|--------|------|
| Strategy 接入注册表 | 3 个 Strategy 文件仍调用 `transformRequest()` 的 switch-case，未使用 `buildProviderRequest()` |
| `transformRequest.ts` 删除 | 1719 行、66 个 switch case，应被 provider 注册表完全替代 |
| `transformResponse.ts` 删除 | 397 行，应被每个 provider 的 ResponseTransform 替代 |
| 响应转换接入注册表 | `buildProviderRequest()` 目前只构建请求，未处理响应转换 |
...
Portkey 的 streaming 也走 `responseTransforms['stream-chatComplete']`。Chainr 当前的 8 个 `transform*Stream.ts` 文件需要评估是否可以被 provider 的 stream transform 替代。

### 4.3 不做的事

- 不改 Strategy 类的 fallback/loadbalance/single 设计 — 这是 Chainr 自己的路由层
...
Portkey 使用动态 `await import()` 加载 provider。Chainr 使用静态导入注册表 `Providers[provider]`。
...
| `src/core/providerRequest.ts` | ✅ 已创建 | Portkey 风格的请求构建器，待扩展多 endpoint |
| `src/core/awsSigV4.ts` | ✅ 已删除 | 被 bedrock/utils.ts (@smithy) 替代 |
| `src/core/transformRequest.ts` | ⬜ 待删除 | Phase 5：被 provider 注册表完全替代 |
| `src/core/transformResponse.ts` | ⬜ 待删除 | Phase 5：被 provider ResponseTransform 替代 |
| `src/core/transform*Stream.ts` (8 个) | ⬜ 待评估 | Phase 5：评估是否可被 provider stream transform 替代 |
