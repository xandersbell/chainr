...
| Realtime scope | Priorai currently exposes OpenAI Realtime bootstrap HTTP surfaces in the SDK, but not a provider-agnostic transport runtime. In the current adapter, `sessions` and `transcription_sessions` use the beta bootstrap header while `client_secrets` does not. |
